...
def CoordinateTransform(xyin, za, mode, inr=0., cent=np.array([[0.],[0.]])):
    ...
    if c.mode=='mcs_pfi' or 'mcs_pfi_wofe':
        xyin=Pixel2mm(xyin, inr, cent,pix=DCoeff.mcspixel,invx=1.,invy=-1.)
    elif c.mode=='mcs_pfi_asrd':
        xyin=Pixel2mm(xyin, inr, cent,pix=DCoeff.mcspixel_asrd,invx=-1.,invy=1.)
...
def ScalingFactor(xyin, c):
    ...
    if c.mode=='mcs_pfi_asrd':
        dist1=[mt.sqrt((i*c.rsc[0]-DCoeff.distc_x_asrd)*(i*c.rsc[0]-DCoeff.distc_x_asrd)+(j*c.rsc[0]-DCoeff.distc_y_asrd)*(j*c.rsc[0]-DCoeff.distc_y_asrd)) for i,j in zip(*xyin)]
        scale=[r*c.rsc[0]-(c.rsc[1]+c.rsc[2]*s+c.rsc[3]*s*s+c.rsc[4]*np.power(s,3.)) for r,s in zip(dist,dist1)]
        #scale=[ScalingFactor_Rfunc(r,c) for r in dist]
    # at the Summit
    else:
        # scale1 : rfunction

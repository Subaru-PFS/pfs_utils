...
"""
Example use:
python BlackDotsScript.py -work_directory /work/ncaplar/BlackDotsData/ -first_name mcs_data_all_1 -second_name mcs_data_all_2 -first_type theta -second_type phi -cobra_id 505
...
class OptimizeBlackDotsVisualize:
    ...
    def __init__(self, mcs_data_all_1, mcs_data_all_2, optimize_black_dots_instance, getCobraGeometry_cobra_id):
        ...
        self.mcs_data_all_1 = mcs_data_all_1
        self.mcs_data_all_2 = mcs_data_all_2
    ...
    def show_single_cobra(self, cobra_id, show_predictions=True, show_optimization=True):
        ...
        # fig.suptitle(f'cobra_id = {cobra_id}') 

        data1 = ax1.scatter(self.mcs_data_all_1[0, cobra_id+1][:40], self.mcs_data_all_1[1, cobra_id+1][:40], color='blue', label='observed in run 1')
        # ax1.plot(poly_deg_x_1(np.arange(0,40)), poly_deg_y_1(np.arange(0,40)), color='blue')
        ax1.scatter(self.mcs_data_all_2[0, cobra_id+1][:40], self.mcs_data_all_2[1, cobra_id+1][:40], color='orange')
        # ax1.plot(poly_deg_x_2(np.arange(0, len(y_measurments_2))), poly_deg_y_2(np.arange(0,len(y_measurments_2))), color='orange')

        ax1.scatter(self.getCobraGeometry_cobra_id.loc[cobra_id+1]['center_x_mm'], self.getCobraGeometry_cobra_id.loc[cobra_id+1]['center_y_mm'], color = 'black')
        cc_cobra = Circle((self.getCobraGeometry_cobra_id.loc[cobra_id+1]['center_x_mm'], self.getCobraGeometry_cobra_id.loc[cobra_id+1]['center_y_mm']), 
                       self.getCobraGeometry_cobra_id.loc[cobra_id+1]['motor_theta_length_mm'], color='black' , alpha=0.25,label='theta radius')
        ax1.add_artist(Circle((self.getCobraGeometry_cobra_id.loc[cobra_id+1]['center_x_mm'], self.getCobraGeometry_cobra_id.loc[cobra_id+1]['center_y_mm']), 
                      self.getCobraGeometry_cobra_id.loc[cobra_id+1]['motor_theta_length_mm'], color='black' , alpha=0.25,label='theta radius'))
        ...
        cc_cobra = Circle((self.getCobraGeometry_cobra_id.loc[cobra_id+1]['center_x_mm'], self.getCobraGeometry_cobra_id.loc[cobra_id+1]['center_y_mm']), 
                      self.getCobraGeometry_cobra_id.loc[cobra_id+1]['motor_theta_length_mm'], color='black' , alpha=0.25, label=r'cobra $\theta$ radius')
        cc_original = Circle((xd_original, yd_original), 0.75, color='red', alpha=0.15, label='original dot position')
        cc_modified = Circle((xd_modified, yd_modified), 0.75, color='red' , alpha=1, label='optimized dot position', fill=False, lw=5)

        ax2.scatter(self.getCobraGeometry_cobra_id.loc[cobra_id+1]['center_x_mm'], self.getCobraGeometry_cobra_id.loc[cobra_id+1]['center_y_mm'], color='black')
        ...
        data1 = ax2.scatter(self.mcs_data_all_1[0, cobra_id+1][:40], self.mcs_data_all_1[1, cobra_id+1][:40], color='blue', label='observed in run 1')
        # ax2.plot(poly_deg_x_1(np.arange(0,40)), poly_deg_y_1(np.arange(0,40)), color='blue')
        data2 = ax2.scatter(self.mcs_data_all_2[0, cobra_id+1][:40], self.mcs_data_all_2[1, cobra_id+1][:40], color='orange', label='observed in run 2')
    ...
    def show_improvment_1d(self):        
        ...
        plt.subplot(131)
        plt.hist(np.array(np.array(self.list_of_total_penalty_for_single_dot_original)),bins=np.arange(-10,40,1))
        plt.title("'Penalty' for the original dot positions")
        plt.ylim(0,1400)
        plt.subplot(132)
        plt.hist(np.array(self.list_of_total_penalty_for_single_dot_optimized),bins=np.arange(-10,40,1))
        ...
        plt.hist(-np.array(np.array(self.list_of_total_penalty_for_single_dot_original)
                                   -np.array(self.list_of_total_penalty_for_single_dot_optimized)),bins=np.arange(-20,10,1))
        plt.ylim(0,1400)
...
# select which dataset and instances of optimization to visualize
# you need to pass BOTH the correct data and the optimization instance
# TODO: make so you do not have to pass _both_ the dataset and the optimization instance
# problem is that optimization instance does not have a way to make avaliable to the outside input data
